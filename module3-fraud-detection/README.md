...
├── data/                               # gitignored — download from Kaggle, or regenerate via notebooks
│   ├── train_identity.csv
│   ├── train_transaction.csv
├── models/                             # committed as CI/test fixtures — all three still reproducible via the notebooks below
│   ├── model.json                      # Trained XGBoost model
│   ├── schema.json                     # Raw input schema for the FastAPI request model
│   └── cols_to_drop.json               # Columns dropped during preprocessing, shared by training and inference
├── notebooks/
...
│   └── api/
│       ├── main.py                     # FastAPI app — loads the model, defines /predict
│       ├── pipeline.py                 # Applies preprocessing to a request and runs inference
│       └── schemas.py                  # Pydantic request/response models
├── tests/
│   ├── test_preprocessing.py           # Unit tests for src/preprocessing.py
│   ├── test_pipeline.py                # Integration test for pipeline.py's _preprocess()
│   └── test_predict_endpoint.py        # Acceptance tests for the /predict endpoint
├── adrs/
│   ├── 001_join_strategy.md
│   ├── 002_chronological_split.md
│   └── 003_model_decision.md
├── docs/
│   └── cost-analysis.md
...
### Generate the model
The files in `models/` are committed as fixtures so tests and the API work out of the box — but they're still build artifacts, reproducible from the notebooks below. Regenerate them in order if you change preprocessing or training:
1. Run `notebooks/01_eda.ipynb` — generates `data/merged.parquet`
2. Run `notebooks/02_preprocessing.ipynb` — generates train/val/test parquet files, `models/schema.json`, `models/cols_to_drop.json`
